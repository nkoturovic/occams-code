...
#
# Run weekly via cron. Removes logs older than RETENTION_DAYS.
# Default: 30 days. Override via env: RETENTION_DAYS=14 ./cleanup-logs.sh
#
# Logs cleaned:
#   ~/.local/share/opencode/oh-my-opencode-slim.*.log  (plugin session logs)
#   ~/.local/share/opencode/log/*.log                  (core session logs)
...
# Only print on action (cron suppresses empty stdout)
